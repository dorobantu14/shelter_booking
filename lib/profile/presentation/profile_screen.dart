...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<SheltersBloc, SheltersState>(
      listener: (context, state) {
        if (state.status == SheltersStatus.loggedOut) {
          context.router.push(const LoginWrapperRoute());
        }
      },
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              onPressed: () async {
                context.read<SheltersBloc>().add(const SheltersEvent.logout());
              },
              icon: Icon(
                Icons.logout,
                size: 32,
